...
class ProxySupervisor {
  constructor({ strategy = defaultStrategy, refreshInterval = null } = {}) {
  ...
  async initialize() {
    ...
    if (Number.isInteger(this.config.refreshInterval)) {
      ...
      setTimeout(() => this.refresh(), this.config.refreshInterval);
    }
  ...
  proxy() {
    ...
    return async (req, res, next) => {
      if (this.started === false) throw Error('supervisor must be initialized first!');
